    ...
    const navigate = useNavigate()
        ...
        async function handleSumbit(){
            if (workoutType === "cardio"){
                await fetch(`${process.env.REACT_APP_WORKOUTS_HOST}/api/cardio_workout`, {method:'POST', headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                  },body:JSON.stringify(cardio)})
            } else{
                await fetch(`${process.env.REACT_APP_WORKOUTS_HOST}/api/strength_workout`, {method:'POST', headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            <div className="bg-grey-lighter min-h-screen flex flex-col">
                <div className="container max-w-sm mx-auto flex-1 flex flex-col items-center justify-center px-2 space-y-1">
                    <div className="bg-white px-6 py-8 rounded shadow-md text-black w-full">
                        <h1 className="mb-8 text-3xl text-center">Add a Workout</h1>
                    ...
                    <button type="button" onClick={handleSumbit} className="w-full bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
